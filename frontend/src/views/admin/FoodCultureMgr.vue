<template>
  <div class="culture-manage">
    <div class="page-header">
      <h1>美食文化管理</h1>
      <button class="add-btn" @click="openAdd">添加美食</button>
    </div>
    ...
    <div class="modal" v-if="showModal">
      <div class="modal-content">
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="form-group">
            <label>标签</label>
            <input type="text" v-model="form.tags" placeholder="如：麻辣 · 特产" />
          </div>
          <div class="form-group">
            <label>图片路径</label>
...
export default {
  ...
  setup() {
    ...
    const form = reactive({ title: '', description: '', tags: '', image: '' })
    ...
    const fetchItems = async () => {
      loading.value = true
      try {
        const res = await publicApi.get('/culture/food')
    ...
    const openAdd = () => {
      ...
      Object.assign(form, { title: '', description: '', tags: '', image: '' })
    ...
    const handleSubmit = async () => {
      try {
        if (editingId.value) {
          await adminApi.put(`/culture/food/${editingId.value}`, form)
        } else {
          await adminApi.post('/culture/food', form)
        }
    ...
    const handleDelete = async (id) => {
      ...
      try {
        await adminApi.delete(`/culture/food/${id}`)
...
.spinner { width: 40px; height: 40px; border: 3px solid #f0f0f0; border-top-color: #ff6b35; border-radius: 50%; animation: spin 0.8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.add-btn { padding: 10px 20px; background: linear-gradient(135deg, #ff6b35, #c41e3a); color: #fff; border: none; border-radius: 6px; font-size: 14px; cursor: pointer; }
...
.tags { font-size: 12px; color: #ff6b35; margin: 4px 0; }
...
.submit-btn { background: linear-gradient(135deg, #ff6b35, #c41e3a); color: #fff; }
