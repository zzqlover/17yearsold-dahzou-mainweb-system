    ...
    <div class="modal" v-if="showModal">
      <div class="modal-content">
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="form-group">
            <label>年份</label>
            <input type="text" v-model="form.year" placeholder="如：1933-1949" />
          </div>
          <div class="form-group">
            <label>图片路径</label>
...
export default {
  ...
  setup() {
    ...
    const fetchItems = async () => {
      loading.value = true
      try {
        const res = await publicApi.get('/culture/red')
    ...
    const openAdd = () => {
      ...
      Object.assign(form, { title: '', description: '', location: '', year: '', image: '' })
    ...
    const handleSubmit = async () => {
      try {
        if (editingId.value) {
          await adminApi.put(`/culture/red/${editingId.value}`, form)
          showToast('更新成功', 'success')
        } else {
          await adminApi.post('/culture/red', form)
    ...
    const handleDelete = async (id) => {
      ...
      try {
        await adminApi.delete(`/culture/red/${id}`)
